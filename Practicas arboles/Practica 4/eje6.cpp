...
template <typename T>
void pertenencia(Abb<T> C,const Abb<T>& D,vector<T>& v)
{
  while(!C.vacio())
  {
    T t=C.elemento();
    Abb<T> aux=D.buscar(t);
    if(aux.vacio())
    {
      v.push_back(t);
    }
...
template <typename T>
Abb<T> Conjunto(const Abb<T>& C,const Abb<T>& D)
{
  vector<T> v;
  pertenencia(C,D,v);
  return AVL(v);
...
//  mostrar(B);
  ////////////////
  cout<<endl;
  //Realizamos la operaciones la Interseccion
  vector<int> v1,v2,v;
  inorden(v1,A);
  inorden(v2,B);
  ...
  //Una vez creados los arboles realizamos la operacion
  cout<<"Union :"<<endl;
  mostrar(C);
  cout<<endl;
